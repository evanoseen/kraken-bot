...
**Update (Day 87):** The floor was deliberately raised to Python 3.10+ — see `ISA.md`'s 2026-09-12 Decisions entry for the full reasoning (production VPS is a stock Ubuntu 24.04 image, ships 3.12; CI already runs 3.11; verified the full suite green under a real 3.11 install before committing). `feedparser` is back to its originally-proposed `>=6.0.14` bound. The `pytest` 9.x and `anthropic` 1.x PRs are still closed, not merged — the floor no longer blocks them, but they're major-version bumps that need their own review pass under points 1-2 above, not a same-day bundle with the floor change itself.
