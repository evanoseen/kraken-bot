...
## Days 66-83 catch-up, 2026-09-07

Same gap as Day 65, again. The backlog entry for this task said "Days 66-79"; by the time it actually got picked, Days 80-83 had shipped too, so this covers the full run through yesterday instead of stopping at a boundary that was already five days stale.

**Shipped, by category:**

- **Meta/backlog maintenance** (66, 79, 82) — Day 66 extended the backlog again (single-entry-at-a-time wasn't sustainable) and, while surveying for what to add, found and fixed two real bugs: `.gitignore` protecting a filename (`latest_status.json`) that was never the one actually shipped, and a live mypy regression where a version bump silently stopped honoring `mypy.ini`'s `follow_imports=silent`. Day 79 found the largest single piece of drift this whole project has produced — see "Biggest surprise" below. Day 82 re-audited the README and found five shipped features (trailing stop, config validation, reconciliation, coverage tooling, Dependabot) completely absent from it.
- **Test coverage sweep** (67, 68, 70, 72, 77, 78, 80, 81, 83) — half of this block, by day count. Docstrings for the remaining modules, a permanent mypy type-hint lock on four more files, Nitter 3-instance failover tests, `pytest-cov` wired up with a `make coverage` target, then a systematic sweep bringing `pump_detector.py` (8%→98%), `listing_monitor.py` (28%→100%), `positions.py` (57%→100%), `kraken_client.py` (75%→100%), and `trader.py` (82%→100%) up in turn — each one grounded in the previous day's actual coverage output, not guessed. Closed out Day 83 with a CI gate (`--cov-fail-under=95`) so none of it can silently erode the way the docs did. Overall coverage: 83% → 98%.
- **Risk/safety features** (69, 73) — a trailing stop that locks in gains on a pullback from a position's peak instead of only a fixed take-profit, and config validation at startup that rejects contradictory tunables (like `MIN_TRADE_AMOUNT` above `MAX_TRADE_AMOUNT`) before the first cycle instead of failing confusingly downstream.
- **Ops/security/dependency hygiene** (71, 74, 75, 76) — upper-bound pins on every dependency, a positions.json↔Kraken reconciliation script (with its own Telegram alert), a full SECURITY.md re-audit, and Dependabot wired up for automated update PRs — confirmed genuinely active by watching it trigger its own workflow run right after the config was pushed.

**Biggest surprise:** Day 79's ISA.md finding dwarfs anything Day 65 turned up. `ISA.md` — the project's own declared system of record, whose first line says "iteration on the bot is iteration on this file" — had `updated: 2026-05-20` in its frontmatter, unchanged through 76 subsequent days of shipped work. 34 of 38 criteria were still marked unchecked despite the features (kill switch, heartbeat, JSONL trades, retry/rate-limit, drawdown breaker, deploy script) having been live for weeks, and one of its verification citations had been factually wrong since Day 62. This isn't a new *kind* of finding — it's the same "shipped code outpaced its own documentation" pattern Day 65 named as the headline surprise for the previous block — but it landed on the single document explicitly designed to prevent exactly that outcome, and it still happened anyway. The fix that actually matters isn't re-auditing ISA.md once; it's that this journal itself just proved the same discipline (one entry per day) doesn't survive contact with momentum either, twice now. A periodic audit *of the audits* — checking whether every "living document" has actually been touched recently — might be the more durable fix than continuing to catch each one individually after the fact.

**Next:** Day 85, ISA v2 — the deferred second wave of ISCs for everything shipped since Day 2 (trailing stop, config validation, reconciliation, coverage tooling, Dependabot, and more), queued explicitly by Day 79 rather than rushed into the same sitting.

---

## Day 90, 2026-09-18

**Shipped:** The anthropic 1.x bump, held back since Day 86/87 specifically because it carries real risk — `market_matcher.py` is the only place this bot calls an LLM to decide what to buy. Read the SDK's actual `MIGRATION.md` first rather than trusting a green test run: v1.0.0's breaking changes are httpx→httpx2 (irrelevant, no custom `httpx.Client` here), removed sampling kwargs on `messages.create()` (irrelevant, none passed), and structured-output arg renames (irrelevant, not used). The one change that sounded scary on first read — raw response `.text()`/`.read()` becoming method calls — turned out to describe the raw-HTTP-response helpers, not the parsed `Message.content` list this codebase actually reads via `content[0].text`. Confirmed the test mock (`tests/test_market_matcher.py`'s `MagicMock`-as-`Message` fixture) still matches reality and needed zero changes. Bumped the pin, installed 1.7.0, full suite green (466 passed, 98% coverage). Then did the part that actually mattered: called `analyze_news_for_trades()` directly against the live API with real headlines and the real tradable-coin list — 6 clean signals came back, correctly shaped, correctly filtered by confidence.

**Surprised by:** Two things. First, `main.py --once --dry-run` alone would have been a false positive here — the live Kraken balance is $0.01 CAD, so the trader's balance check short-circuits the cycle before it ever reaches the news-analysis step, meaning a "dry-run cycle" can exit 0 and log "READY" without the anthropic call path being touched at all. Had to go around the trader entirely and call `market_matcher` directly for a real end-to-end check. Second, mypy — never run on this file before today, so not a regression, just a first look — failed immediately on `message.content[0].text`: the 1.x `ContentBlock` union expanded to roughly ten block types, most without `.text`. The existing `except Exception` already made this runtime-safe, but "safe by accident via a broad catch-all" isn't the same as "correct," so added a real `isinstance` guard, rebuilt the test fixture around a genuine `anthropic.types.TextBlock` instead of an untyped `MagicMock`, and covered the rejection path.
...
**Shipped:** `tests/test_remaining_modules_types.py` — one mypy subprocess invocation plus the same AST full-annotation audit `test_config_notifier_status_blacklist_types.py` uses, across all 16 modules Day 91 flagged as clean-but-unlocked (`coin_trade_counter.py`, `cooldown.py`, `cycle_timer.py`, `headline_cache.py`, `health.py`, `heartbeat.py`, `kill_switch.py`, `listing_monitor.py`, `main.py`, `news_fetcher.py`, `portfolio.py`, `positions.py`, `pump_detector.py`, `retry.py`, `signals.py`, `trade_logger.py`). The mypy half confirmed Day 91's claim immediately — all 16 were already clean. The AST half didn't: six functions across five files (`health.py`'s three `Config`-taking helpers, `listing_monitor.py::save_seen`, `positions.py`'s `save_positions`/`record_buy`/`remove_position`, `pump_detector.py::find_pumping_coins`, `portfolio.py::compute_value`) had bare or defaulted untyped parameters that mypy happily inferred as `Any` around, which is exactly the "clean" that isn't the same claim as "annotated" Day 68 already ran into once with `status.py`/`blacklist.py`. Added the missing annotations before writing the lock test around them rather than locking in the gap. `portfolio.py::compute_value` was the one genuinely tricky case: giving `get_price_fn` a real `Optional[Callable[...]]` type broke the existing pattern of reassigning the parameter itself inside a conditional local import — mypy couldn't narrow away `None` through that reassignment even with a `# type: ignore[assignment]` already there, so it flagged the later call as `"None" not callable`. Swapped to a separately-declared `resolver` local instead of reassigning the parameter, which narrows cleanly with no ignore comment. Verified with the CI-equivalent command directly (`pytest -v --cov=. --cov-report=term-missing --cov-fail-under=95`): 471 passed, 98.14% coverage, gate held, `pip check` clean.

**Surprised by:** How consistently "mypy clean" undersells the actual gap. This is the second time in two days (`market_matcher.py` on Day 90/91, now six functions across five more files today) that a file mypy passed on wasn't actually the "fully typed" state the backlog's Done-when bar assumes it is — because an untyped parameter that's never compared against an incompatible type, or a missing return annotation on a function that only ever returns implicitly, gives mypy nothing to flag. The lock tests this project keeps writing (`test_kraken_client_types.py`, `test_trader_types.py`, `test_config_notifier_status_blacklist_types.py`, `test_market_matcher_types.py`, now this one) are doing real work specifically because they pair the mypy invocation with the AST audit — either check alone would have called today's task done six functions early.

**Next:** Day 93 (signal-driven exit on a stale buy thesis — the one open item in `STRATEGY.md`'s structural-weaknesses list) is queued next, followed by Day 94 (raise the CI coverage gate off its Day 83 floor now that 98%+ has held stable for eleven days).

---

## Day 93, 2026-09-21

**Shipped:** The one real open item left in `STRATEGY.md`'s structural-weaknesses list since Day 66: a held position's original catalyst was never re-checked, only price thresholds could close it. Added `check_signal_reversal_exits` (`trader.py`) — runs once per cycle, right after pump+news signals are merged, and exits any held coin whose signal batch this cycle carries a fresh `sell` for it, logged as `sell_signalreversal`, distinct from `sell_stoploss`/`sell_takeprofit`/`sell_trailingstop`/`sell_stale`. It fires on the thesis, not on P&L — a position can be net positive and still get exited if the reason it was bought no longer holds.

**Surprised by:** How much the "just add a check" framing in the Do line undersold. Wiring it in surfaced two real correctness issues before I even got to writing the intended test: first, my initial version trusted a post-check `get_holdings()` refresh to keep the generic signal loop from re-processing the same sell — two existing live-mode tests (with a static `get_holdings` mock) caught a genuine double-sell / double-counted-P&L bug that a real Kraken account might paper over with fast-enough balance updates but that I didn't want resting on an assumption for live money. Fixed by having the new check return which coins it claimed and filtering them out of the signal list directly, correct regardless of API timing. Second, claiming a coin before checking whether it had a *tracked* position (vs. just being Kraken-held) would have silently swallowed the sell signal for exactly the kind of untracked-position drift Day 74's reconciliation script exists to catch — fixed by only claiming after confirming a real position and price are available. The reordering also turned a chunk of the generic loop's own live-sell branch permanently unreachable (any tracked-position coin is now claimed upstream first), which would have quietly dropped `trader.py` below its Day 81/92 100%-coverage floor if left in — simplified that branch to the one case that can still reach it and added a test naming it. None of this changed the actual feature I set out to build; all of it was making sure "distinct exit path" didn't mean "a second, racier path to the same bug."

**Next:** Day 94 (raise the CI coverage gate off its Day 83 floor — `trader.py` is now back at 100% and overall coverage sits at 98.18%, comfortably past the 95% floor that's been unmoved since Day 83).

---

## Day 94, 2026-09-22

**Shipped:** Raised the CI coverage gate (`.github/workflows/test.yml`'s `--cov-fail-under`) from 95 to 96, the last open item Day 91's sweep queued. Verified for real rather than eyeballing the current percentage against the old threshold: built a fresh Python 3.11 venv (system Python has neither pytest nor coverage, same as every prior day that needed them), ran the exact CI command, got 478 passed at 98.18% overall — matching Day 93's own number exactly, confirming coverage really has held since then rather than drifted either direction. Picked 96 to keep roughly Day 83's original headroom ratio (95 against a 98% baseline) rather than just splitting the difference. Proved the new gate actually gates: reran with `--cov-fail-under=96` clean, then did the synthetic-drop check the Do line asked for — moved `tests/test_trader_coverage_gaps.py` out of the tree, reran, watched `trader.py` fall to 86% and overall to 95.10%, and watched the gate correctly fail with "Required test coverage of 96% not reached." Restored the file and confirmed `git status` clean before touching anything else. Also updated the `Makefile` coverage-target comment and `ISA.md`'s ISC-43 (checklist line and Test Strategy row) to the new number so the threshold doesn't end up documented in three places that quietly disagree — checked `STRATEGY.md` and `SECURITY.md` too; neither mentions a coverage percentage, so nothing to touch there.

**Surprised by:** Not the coverage work itself — that went exactly as scoped — but what I found while looking for today's task in the first place. `DAILY_ITERATIONS.md`'s own "## Day 94: ..." heading was missing: Day 93's long Result paragraph ran straight into Day 94's `**Why:**` line with nothing marking the boundary, so a `grep '^## Day'` sweep (the exact method Day 91's sweep, and this run's own first step, both use to find "the lowest-numbered Day without a Result") would silently skip right past it. This is the fourth or fifth time this project's own tracking files have been the thing that drifted rather than the code (`ISA.md`'s stale frontmatter on Day 79, missing Result paragraphs on Day 91, a stale "Next:" pointer the same day) — this time it wasn't stale content, just a dropped heading, but the failure mode is the same: the file that's supposed to be the reliable record needs its own integrity checked, not just trusted.

**Next:** No task is queued past Day 94 — this run's own sweep (grep for `^## Day` with no matching `**Result:**` block, cross-checked against the Days-93/94 boundary bug just found) turned up nothing else open. Whoever picks up Day 95 should do a fresh survey pass like Days 66/79/91 did, rather than assume the backlog still has unclaimed entries sitting in it.

---

## Day 95, 2026-09-23

**Shipped:** Did the fresh survey Day 94 asked for instead of assuming a queued task existed. Re-ran the full suite first to confirm Day 94's numbers still held (they did: 97.76% coverage, same 7 known `.env`-related `test_cli_flags.py` failures), then ran a mypy sweep — but this time actually pointed it at `scripts/`, which every prior sweep (Days 15, 16, 68, 90, 91, 92) had quietly limited to the repo-root modules. First run found two real `var-annotated` errors in `scripts/archive_trades.py` (`archive_csv`/`archive_jsonl` both build their kept/archived lists via bare `x, y = [], []`, which mypy can't infer a type for). Fixed both with explicit `list[...]` annotations, confirmed the other three scripts were already clean, ran the AST full-annotation audit (clean across all four), and shipped `tests/test_scripts_types.py` to lock the whole directory in one go — mirroring `tests/test_remaining_modules_types.py`'s shape rather than inventing a new one.

**Surprised by:** How the character of the finding differed from the last five rounds of this exact exercise. Days 90-92 kept finding files that were *already mypy-clean* with nothing pinning them there — a documentation-style gap, "true today, not proven to stay true." This time was different: `scripts/archive_trades.py` had a live, currently-failing mypy error sitting in a file nobody had ever pointed mypy at. Nothing about the code was wrong at runtime (the lists work fine untyped), but it's a reminder that "we mypy-sweep this project regularly" and "we mypy-sweep the whole project regularly" have quietly been two different claims since Day 15 — the `scripts/` directory existed before most of those sweeps did (Day 59) and just never made it into any of their target lists.

**Next:** Day 96 — checking README/STRATEGY.md/SECURITY.md for the same kind of drift this project keeps finding turned up a real, live one: `STRATEGY.md` and `SECURITY.md` are current, but `README.md`'s Features list (last regenerated Day 82) has no line for the Day 93 signal-reversal exit, its one exit path that isn't price-driven. Didn't fold that fix into today's commit — this file, and today's actual change, are about `scripts/` typing, and bundling in an unrelated README edit would've been the same scope-creep this project's own "Do" lines warn against. Queued as Day 96 in `DAILY_ITERATIONS.md` instead.

---

## Day 96, 2026-09-24

**Shipped:** The README fix Day 95 queued, plus the rest of the Day-82-style full audit rather than stopping at the one known gap. Added the Day 93 signal-reversal exit to the Risk management bullet list and as its own step in the "How It Works" numbered flow (ordered before the generic signal-merge step, matching `trader.py`'s actual per-cycle order from Day 93's double-sell fix). Then re-ran the CI-equivalent command for real current numbers instead of trusting the last-written record — 473 passed, same 7 known `.env`-related `test_cli_flags.py` failures, 97.76% coverage, 96% gate held, matching Day 95 exactly — and found the README's test-count line ("52 test files / 466 tests") had drifted well behind reality: `pytest --collect-only` shows 480 tests across 57 files now. Fixed that in both places it appears (Project Structure tree, Tech Stack list). Two more real gaps surfaced on the same pass: `.env.example` documents 27 variables now, not the 26 the Setup section claims (the file itself stays correctly synced to `config.py` via `tests/test_env_example.py` — only the prose count was stale), and the Tech Stack's mypy line still named the original six locked files from Days 15/16/68, three major lock-test batches out of date — `ls tests/*types*.py` shows six lock-test files covering all 27 source files now (root + `scripts/`, per Days 90-95). Reworded that line to describe the mechanism instead of an enumerated file list, so a seventh lock-test file landing later can't silently make the README wrong again the same way.

**Surprised by:** How many small counts had drifted in exactly six days (Day 82's last full audit) even though nothing about the *shape* of the project changed — no new feature category, just steady test-count and file-count growth from the mypy-locking work (Days 90-95) that a features-focused audit wouldn't naturally think to re-check. The one-known-gap framing in Day 95's "Next:" note undersold what a real full pass would turn up, same lesson as Day 82's own note about Day 64 having only caught the obvious gap the first time. Also notable: this is the first day in a while that didn't need a from-scratch Python 3.11 venv build — this sandbox's system Python is already 3.11.15 — so the "build a venv" ritual most Result notes since Day 87 open with wasn't actually necessary today, just `python3 -m venv` against the system interpreter directly.

**Next:** No specific task is queued past Day 96. `DAILY_ITERATIONS.md`'s own backlog has run dry at this point several times before (Days 55, 66, 79, 91, 94→95) — whoever picks up Day 97 should do the same kind of fresh survey pass (re-run the suite for real numbers, mypy-sweep, diff the docs against the code) rather than assume an unclaimed entry is sitting in the file.

---

## Day 97, 2026-09-25

**Shipped:** Did the fresh survey Day 96 asked for — suite and mypy sweep both came back clean, no drift there this time — but the actual gap turned up on the GitHub side: 5 Dependabot PRs (`anthropic`, `krakenex`, `types-requests`, `python-dotenv`, `pytest-cov`) had been sitting open since 2026-09-14, about 11 days, well past the weekly cadence `OPS_RUNBOOK.md` section 10 documented after Day 86's first 2+ week backlog. Reviewed and closed out all 5 individually, same discipline as Day 86: 4 merged cleanly through GitHub with CI already green on each PR's own head commit; the `anthropic` one reported a "dirty" merge state from an 11-day-stale branch despite a single non-conflicting line, so applied the identical bump directly to `main` (same move Days 86/89/90 made for PRs #1 and #3) and closed the PR with a comment pointing to the commit. None of the 5 crossed an already-pinned major-version ceiling, so this didn't need the full migration-guide read Day 90's `anthropic` 1.x jump did — but read the 1.6.0 release notes anyway since it's the one dependency with real safety weight, and confirmed nothing in it touches the `Message`/content-block surface `market_matcher.py` depends on. Verified before and after every merge: full suite + coverage gate (473 passed, 97.76%, matching Day 96 exactly both times), mypy clean across all 27 files, `pip-audit` clean, and — new this time — actually checked GitHub's own Actions runner status via the GitHub MCP tools instead of only the local venv proxy, closing a small piece of the "not verified on GitHub's own runner" gap this project's Result notes have flagged honestly since Day 56.

**Surprised by:** How much of this task was just discovering it existed. The code itself had zero drift today — first time in a while a fresh survey's suite-and-mypy pass came back completely clean — so the entire task lived in a place none of the local verification commands would ever surface: an open-PR count on GitHub that nothing in this sandbox checks by default. Also mildly surprised the `anthropic` PR's branch went stale enough to conflict on a single-line diff; Dependabot apparently doesn't auto-rebase an open PR against a fast-moving `main`, which is exactly the same shape of problem as the cadence gap itself — a mechanism that opens something and then never revisits it.

**Next:** No specific task is queued past Day 97. Fresh survey pass again for whoever picks up Day 98 — but this run's own finding suggests it's worth widening what "survey" means beyond the suite/mypy/docs triad Days 66/79/91/94/95/96 have repeated: check the open-PR count too, since that's exactly the kind of gap none of the other checks would ever catch.

---

## Day 98, 2026-09-26

**Shipped:** Did the widened survey Day 97 asked for — open-PR count (0), CI status (green on the latest commit), full suite, mypy sweep, `.env.example` sync, `pip-audit`, and a stale-language grep across every living doc all came back completely clean, no drift anywhere, first time that's happened across this many checks at once. Rather than stop there, looked at *why* the remaining coverage misses existed instead of just reading the percentage, and found `health.py` sitting at 88% for a reason that mattered: every test in `tests/test_health.py` mocks `_check_kraken_connectivity` itself, so the function's actual `requests.get` call and its response/exception handling had never run under test — the same "mocked out everywhere, never actually exercised" shape Days 77/78 found in `pump_detector.py`/`listing_monitor.py`/`positions.py`, just at the scale of one function instead of a whole module, which is exactly why the module-level sweeps never caught it. Added 5 direct tests covering the ok/non-ok/Kraken-error-field/exception/malformed-JSON cases. `health.py` went 88% → 100%, overall 97.76% → 98.11%, 478 passed, mypy and `pip check` both clean.

**Surprised by:** How a completely clean survey almost became "nothing to report" instead of a real task. Every prior dry-survey day (66, 79, 91, 94→95, 95→96, 96→97) found its next task in a place the standard checks didn't look — a status-note gap, a stale heading, an unaudited directory, a doc count, an unreviewed PR queue. This time the standard checks really were all clean, and the actual gap was one level more subtle: not "is this file covered" but "does the coverage number reflect real exercise of the code, or a mock standing in for it." Worth naming as its own recurring category now — "coverage percentage without checking why the remaining misses exist" — since it's now found real gaps twice (Days 77/78's module-scale version, today's function-scale one) and there's no reason to think it's the last time.

**Next:** No specific task queued past Day 98. Given today's finding, whoever surveys next might get more out of spot-checking a few of the other sub-100% files' *reasons* (`scripts/daily_pnl.py` at 91%, `notifier.py` and `main.py` at 95% each) for the same "mocked past the real logic" pattern, rather than assuming a low-90s number is just untested edge cases.
