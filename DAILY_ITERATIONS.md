...
## Status note (added Day 98, 2026-09-26)

Day 97's own "Next:" line said to widen the survey beyond the suite/mypy/docs triad to include the open-PR count, so did that first: 0 open PRs (confirmed via the GitHub MCP tools), latest commit green on GitHub's own Actions runner. Then re-ran everything else the prior survey days check — full suite (473 passed, same 7 known `.env`-related `test_cli_flags.py` failures, 97.76% coverage, 96% gate held, matching Day 97 exactly), a full mypy sweep across all 27 source files (clean), `.env.example` vs `config.py` sync (27/27, matches README's prose count), `pip-audit` (no known vulnerabilities), and a grep across `README.md`/`STRATEGY.md`/`SECURITY.md`/`OPS_RUNBOOK.md`/`ISA.md` for stale "planned"/"will add"/"not yet" language (the only two hits — `SECURITY.md`'s hardening-backlog note and `ISA.md`'s ISC-31 — both correctly describe the one genuinely-still-blocked item, Day 56's VPS-access gap, not silent drift). First time in a while every one of those came back completely clean with nothing to fix.

So widened further rather than writing "nothing to do" — the per-file coverage table (still visible from the suite run above) named `health.py` at 88% as the lowest fully-covered-elsewhere file, missing exactly its `_check_kraken_connectivity` function's body (`requests.get`/response-shape/exception handling). Checked `tests/test_health.py`: every single test patches `health._check_kraken_connectivity` wholesale — the actual network-call function has never been exercised by anything. That's the identical shape of gap Days 77/78 found in `pump_detector.py`/`listing_monitor.py`/`positions.py` ("every call site mocks it out entirely"), just smaller in scope (one function, not a whole module) and not caught by any prior sweep because those sweeps were reading the coverage *percentage*, not asking why the remaining misses existed. Picked this as today's task rather than inventing something unrelated from scratch, same reasoning Days 95/97 used.

## Day 98: direct tests for health.py's `_check_kraken_connectivity`
**Why:** Every test in `tests/test_health.py` mocks `health._check_kraken_connectivity` itself rather than calling it — its real `requests.get`/response-shape/exception-handling logic (the only network call this module makes) has never actually run under test, leaving `health.py` at 88% coverage (lines 41-45, the function's entire body) and the exact "mocked everywhere else" blind spot Days 77/78 found at module scale.
**Do:** Add tests that call `health._check_kraken_connectivity()` directly, patching `health.requests.get` instead of the function itself, covering: a healthy 200 response, a non-`ok` HTTP response, Kraken's own `error` field being non-empty on an otherwise-200 response, a `requests` exception (connection failure/timeout), and a malformed/non-JSON response body.
**Done when:** `make coverage` (or the CI-equivalent `pytest --cov`) shows `health.py` at 100%, and the full suite still passes.

**Result (2026-09-26):** Added 5 tests to `tests/test_health.py` calling `_check_kraken_connectivity()` directly against a mocked `health.requests.get`, covering the ok/error-field/non-ok/exception/malformed-JSON cases named in the Do line. All five were genuinely new coverage, not redundant with the existing mocked-out tests above them (which only ever verify `run_checks()`'s branching, never the function itself). Verified with the CI-equivalent command in the existing Python 3.11 venv (system Python here is 3.11.15, no rebuild needed, same as Days 96/97): `pytest -q --cov=. --cov-report=term-missing --cov-fail-under=96` → 478 passed (473 + 5 new), same 7 pre-existing `test_cli_flags.py` failures (missing `.env`, confirmed unrelated every prior day this came up), `health.py` now 100% (was 88%), overall coverage 97.76% → 98.11%, gate held comfortably. `python3 -m mypy --ignore-missing-imports *.py scripts/*.py` clean across all 27 files — `health.py`'s existing `tests/test_config_notifier_status_blacklist_types.py`-style type lock needed no changes since nothing about the module's signatures changed, only its test coverage. `pip check` clean.

Not verified from this sandbox (same standing category as Day 56/57/86/87/90/94/95/96/97): whether this passes on GitHub Actions' own runner once pushed, rather than just the local venv proxy — will confirm via the GitHub MCP tools after pushing, same as Day 97 did for its own commits.
