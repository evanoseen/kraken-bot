...
- Python 3.10+ (per requirements.txt; raised from 3.8+ Day 87 — the Hetzner image ships 3.12)
...
- 2026-09-12 **decision:** Raised the project's Python floor from 3.8+ to 3.10+ (Day 87), without live SSH access to the VPS to confirm its installed interpreter. Reason: `OPS_RUNBOOK.md` already documents the server as Hetzner's stock Ubuntu 24.04 image, which ships Python 3.12 by default — a fact that doesn't require a live login to know, since it's a property of the documented OS release, not of anything mutable on the box. CI already runs 3.11, so both the deploy target and the test pipeline clear a 3.10 floor with room to spare; only this project's local dev docs and pin were stale. Verified rather than assumed: built a real Python 3.11 venv (`/opt/homebrew`), installed `requirements.txt` with `feedparser` restored to Dependabot's originally-proposed `>=6.0.14` (the exact bump Day 86 had to revert), and ran the full suite plus the coverage gate — 466 passed, 98.06% coverage, both clean. Deliberately did NOT reopen or merge the closed `pytest` 9.x or `anthropic` 1.x PRs in the same sitting — those are major-version bumps needing their own scrutiny (`anthropic`'s tests are fully mocked; a mock passing proves nothing about the real 1.x surface). Queued as their own backlog days rather than bolted onto today's floor change.
